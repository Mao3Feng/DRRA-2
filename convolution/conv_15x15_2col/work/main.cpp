 ...
 */
void init() {
#define N 16 * 16 // size of input file
...
#define N 16 * 16 // size of input file
  ...
  int a_size = 15; // matrix a: 15*15
  int a_1row_line = (a_size-1)/16 + 1; // 1 row of a uses 1 rows of RF
  int b_size = a_size-2; // filter: 3*3, matrix b: 13*13
  int b_1row_line = (b_size-1)/16 + 1; // 1 row of b uses 1 rows of RF
  int b_line = ((b_size-1)/16 + 1) * b_size; // matrix b uses 13 rows of RF
  ...
  for (int i = 0; i < b_size; i++) { // index of row, line0,1,2, line1,2,3 ... line12,13,14 of a
    for (int j =0; j < b_size; j++) { // index of column, b[0], b[1], ... b[12]
