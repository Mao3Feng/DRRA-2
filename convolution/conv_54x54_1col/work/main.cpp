 ...
 */
void init() {
#define N 217 * 16 // size of input file
...
#define N 217 * 16 // size of input file
  ...
  int a_size = 54; // matrix a: 54*54
  int b_size = a_size-2; // filter: 3*3, matrix b: 52*52
  int b_1row_line = (b_size-1)/16 + 1; // 1 row of b uses 4 rows of RF
  int b_line = ((b_size-1)/16 + 1) * b_size; // matrix b uses 208 rows of RF
  ...
  for (int i = 0; i < b_size; i++) { // index of row, line0,1,2, line1,2,3, ... line51,52,53 of a
    for (int j =0; j < b_size; j++) { // index of column, b[0], b[1], ... b[51]
      for (int k = 0; k < 9; k++) { // index of filter, f[0], f[1], ... f[8]
        int row_offset = k / 3;
        int col_offset = k % 3;
        b[64*i + j] += a[b_1row_line * 16 * (i + row_offset) + (j + col_offset)] * f[k];
      }
    }
  }
