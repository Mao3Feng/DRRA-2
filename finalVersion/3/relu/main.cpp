...
#define N 2704 * 16 // size of input file
  ...
  for (int i = 0; i < a_size; i++) { //row
    for (int j = 0; j < a_size; j++) { //column
      v[16 * a_1row_line * i + j] = (16 * a_1row_line * i + j) % 32;
    }
  }
...
void model_l0() {
  ...
  int a_size = 208; // matrix a: 208*208
  int b_size = 208; // matrix b: 208*208
  int b_1row_line = (b_size-1)/16 + 1; // 1 row of b uses 13 rows of RF
  int b_line = b_1row_line * b_size; // matrix b uses 2704 rows of RF
  ...
  for (int i = 0; i < b_size; i++) { //row
    for (int j = 0; j < b_size; j++) { //column
      if (a[16 * b_1row_line * i + j] < 0) {
        b[16 * b_1row_line * i + j] = 0;
      }
      else {
        b[16 * b_1row_line * i + j] = a[16 * b_1row_line * i + j];
      }
    }
  }
