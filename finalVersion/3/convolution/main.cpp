 ...
 */
void init() {
#define N 2941 * 16 // size of input file
 ...
 */
void model_l0() {
  int a_size = 210; // matrix a: 210*210
  int a_1row_line = (a_size-1)/16 + 1; // 1 row of a uses 14 rows of RF
  int b_size = a_size-2; // filter: 3*3, matrix b: 208*208
  int b_1row_line = (b_size-1)/16 + 1; // 1 row of b uses 13 rows of RF
  int b_line = ((b_size-1)/16 + 1) * b_size; // matrix b uses 2704 rows of RF
  int b_num  = b_line * 16; // matrix b uses 2704*16 data in RF
  int c_line = 20*b_1row_line;
  int c_num  = c_line * 16;
  ...
  // Initialize matrix b & c
  for (int ii = 0; ii < b_num; ii++) {
    b[ii] = 0;
  }

  for (int i = 0; i < b_size; i++) { // index of row, row0,1,2, row1,2,3, ... row207,208,209 of a
    for (int j =0; j < b_size; j++) { // index of column, b[0], b[1], ... b[209]
      for (int k = 0; k < 9; k++) { // index of filter, f[0], f[1], ... f[8]
        int row_offset = k / 3;
        int col_offset = k % 3;
        b[16*b_1row_line*i + j] += a[a_1row_line * 16 * (i + row_offset) + (j + col_offset)] * f[k];
      }
    }
  }
