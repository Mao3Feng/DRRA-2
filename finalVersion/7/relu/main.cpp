 ...
 */
void init() {
#define N 208 * 16 // size of input file
  ...
  int a_size = 52; // matrix a: 52*52
  int a_1row_line = (a_size-1)/16 + 1; // 1 row of a uses 4 rows of RF
  ...
  for (int i = 0; i < a_size; i++) { //row
    for (int j = 0; j < a_size; j++) { //column
      v[16 * a_1row_line * i + j] = 16 * a_1row_line * i + j;
    }
  }
